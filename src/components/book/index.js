...
const Book = ({book, library, addToLibrary, removeFromLibrary}) => {
    const isFavorite = Boolean( library.find(libraryBook => libraryBook.id === book.id) );
    ...
    return (
        <div className="book-component flex" id={`book-${book.id}`}>
            <div className="book-thumb">
                ...
                <div style={{background: `url("${book.thumb}")`}} className='book-overlay'></div>
            </div>
            <div className="book-info flex">
                ...
                <div className="search-block__container flex">
                    <a href={book.link} target="_blank" rel="noreferrer" className="search-block__link book-read-link">
